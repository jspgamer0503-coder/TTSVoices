...
def _read_text(path: str) -> str:
    ...
    return raw.decode(_detect_enc(raw), errors="replace")
    ...
    try:
        ...
        try:
            ...
            if "content type" in str(first).lower() or "not a Word file" in str(first).lower():
                # Template file - patch content type
...
def extract_html(path: str) -> str:
    raw  = open(path, "rb").read()
    html = raw.decode(_detect_enc(raw), errors="replace")
    try:
        from bs4 import BeautifulSoup
...
def extract_rtf(path: str) -> str:
    try:
        ...
        return rtf_to_text(open(path, "r", errors="replace").read())
    except ImportError:
...
def _decrypt_odt_inline(path, password, mf_bytes):
    ...
    def aes_gcm(key, iv, ct):
        # IV may be prepended to ciphertext (LibreOffice modern format)
        if len(ct) > len(iv) and ct[:len(iv)] == iv:
            ct = ct[len(iv):]
        try:
            from Crypto.Cipher import AES
...
def _extract_odt_file(path: str) -> str:
    """
    Extract all text from an ODT/ODF file.
    Handles: standard ODT, flat ODF, missing content.xml,
    alternative paths, and partially-readable encrypted files.
    """

    # ── Method 1: Try reading all candidate XML paths ────────────────────
    def _try_xml():
        with zipfile.ZipFile(path, "r") as z:
            names = z.namelist()
            bug_tracker.info(f"ODT zip contains: {names[:10]}")

            # Collect every XML source that might hold text
            # LibreOffice may use content.xml, Body.xml, or word/document.xml
            candidates = [
                "content.xml",          # standard ODF
                "word/document.xml",    # some hybrid formats
                "Body.xml",             # older OOo
                "content/content.xml",  # rare nested
            ]
            # Also scan for any .xml file under root that isn't metadata
            skip_prefixes = ("META-INF/", "Thumbnails/", "manifest", "settings",
                             "meta.xml", "mimetype", "styles.xml")
            for name in names:
                if name.endswith(".xml") and not any(name.startswith(s) for s in skip_prefixes):
            ...
            for cand in candidates:
                if cand in names:
                    try:
                        data = z.read(cand).decode("utf-8", errors="replace")
                        if len(data) > 50:  # Skip tiny/empty files
                            xml_sources.append(data)
                    except Exception as e:
                        bug_tracker.warning(f"ODT: could not read {cand}: {e}")

            if not xml_sources:
                # Check if file is actually encrypted (manifest scan)
                if "META-INF/manifest.xml" in names:
                    try:
                        mf = z.read("META-INF/manifest.xml").decode("utf-8", errors="replace")
                        if "encryption-data" in mf:
                            raise RuntimeError(
                                "ODT_ENCRYPTED"  # sentinel: caller should re-try with password
                            )
                    except RuntimeError:
                        raise
                    except Exception:
                        pass
                raise RuntimeError("No readable XML content found in ODT")
            return xml_sources

    try:
        xml_list = _try_xml()
        all_text = []
        for xml_src in xml_list:
            text = _xml_to_text(xml_src)
            if text.strip():
                all_text.append(text)
        if all_text:
            result = "\n\n".join(all_text).strip()
            bug_tracker.info(f"ODT XML method: {len(result):,} chars")
            return result
    except Exception as e:
        bug_tracker.warning(f"ODT XML method failed: {e}")

    # ── Method 2: odfpy (optional, tries if XML gave nothing) ────────────
    try:
        from odf.opendocument import load
        try:
            from odf import teletype
            doc  = load(path)
            text = teletype.extractText(doc.body).strip()
            if text:
                bug_tracker.info(f"ODT odfpy: {len(text):,} chars")
                return text
        ...
        def _nodes(node):
            ...
            for c in getattr(node, "childNodes", []):
                out.extend(_nodes(c))
            return out
        parts = ["".join(_nodes(e)).strip() for e in doc.body.childNodes]
        ...
        bug_tracker.warning(f"ODT odfpy failed: {e}")

    # ── Method 3: Flat ODF (.fodt) — single XML file ──────────────────────
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as f:
            raw = f.read(512)
        if "opendocument" in raw.lower() or "<office:" in raw:
            with open(path, "r", encoding="utf-8", errors="replace") as f:
                flat_xml = f.read()
            text = _xml_to_text(flat_xml)
            if text.strip():
                bug_tracker.info(f"ODT flat format: {len(text):,} chars")
                return text
    except Exception as e:
        bug_tracker.warning(f"ODT flat format attempt failed: {e}")

    raise RuntimeError(
        "Could not extract text from this ODT file.\n"
